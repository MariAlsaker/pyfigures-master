...
cur_arc_neg = magpy.current.Line(50, vertices_curr_arc_neg)
cur_line_pos = magpy.current.Line(50, vertices_curr_line_pos)
cur_arc_pos = magpy.current.Line(50, np.flip(vertices_curr_arc_pos, axis=0))
cur_line_neg = magpy.current.Line(50, np.flip(vertices_curr_line_neg, axis=0))
...
special_single_loop = MRIcoil.MRIcoil(current=100, diameter=90, custom_coil=True, custom_coil_current_line=the_coil)
...
special_single_loop.show_coil(ax=ax2)
#special_single_loop.show_field_lines(slice="z72", ax=ax2, fig=fig2)
special_single_loop.show_field_lines(slice="y50", ax=ax3, fig=fig2, colorbar=False)
ax3.hlines(y=[30], xmin=[-45], xmax=[45], colors=['k'], linestyles='dashed')
X, Y, Z = plane_at("z=30")
ax2.plot_surface(X, Y, Z, alpha=.3, label="x = 0, slice 50")
