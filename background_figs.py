...
def plot_magn_phase(ax, freqs, magns, phases, colors1, colors2, styles1, styles2, name="RLC"):
    axtwin = ax.twinx()
    for i, magn in enumerate(magns):
        plots = ax.plot(freqs, magn, label=f"Impedance {i+1}", color=colors1[i], linestyle = styles1[i])
    ax.set_ylabel("Impedance [\u03A9]")
    ax.legend(loc="upper left")
    ax.xaxis.set_major_formatter(ticker.FormatStrFormatter('%.2f'))
    #plots[0].set(color = colors1[0])
    for i, phase in enumerate(phases):
        plots = axtwin.plot(freqs, phase/np.pi*360, label=f"Phase {i+1}", color=colors2[i], linestyle = styles2[i])
...
plot_magn_phase(ax, frequencies, magns, phases, colors1, colors2, styles1=linestyle_tuple[:3], styles2=linestyle_tuple[4:7], name=f"Impedance in a parallel RLC circuit,"+names[0]+names[1]+names[2])
#fig.savefig(f"ZparallelRLC", dpi=300 ,transparent=True)
plt.show()
plt.close("all")
...
for i in range(3):
    ...
    phases.append(phase)
fig, ax = plt.subplots(1,1, figsize=(9, 7))
plot_magn_phase(ax, frequencies, magns, phases, colors1, colors2, styles1=linestyle_tuple[:3], styles2=linestyle_tuple[4:7], name=f"Impedance in a series RLC circuit,"+names[0]+names[1]+names[2])
#fig.savefig(f"ZseriesRLC", dpi=300 ,transparent=True)
plt.show()
plt.close("all")
...
ax.grid(True)
#plt.savefig("pnjunk.png", dpi=300 ,transparent=True)
plt.show()
...
ax.plot(ts, longitude, label="Longitudinal", color=cmap(0.3), linestyle=linestyle_tuple[0])
ax.plot(ts, transverse, label="Transverse", color=cmap(0.7), linestyle=linestyle_tuple[4]) 
ax.hlines(y=M0, xmin=0.00, xmax=3.0, colors="k", linestyles="--", label="M0")
#ax.set_title("The Bloch equations applied in the white matter of the brain")
ax.set_xlabel("Seconds [s]")
ax.set_ylabel("Magnetixation [J/T]")
plt.grid(True)
plt.legend()
#fig.savefig(f"theoretical_relax", dpi=300 ,transparent=True)
plt.show()