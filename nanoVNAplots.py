...
mean_data1, std_data1 = find_mean_s11(num_files=10, file_path = folder_pocketvna+"Single_loop/", filename="idun4na", ending=".s2p")
freqs1, reals1, ims1, magn1_db, phase1 = mean_data1[0], mean_data1[1], mean_data1[2], mean_data1[3], mean_data1[4]
mean_data2, std_data2 = find_mean_s11(num_files=10, file_path = folder_nanovna+"single_loop/2MHz/", filename="ketchup4na", ending=".s1p")
...
c1 = "steelblue"
c2 = "orange"
#plot_magn_phase(ax1, [freqs1, freqs2], [magn1_db, magn2_db], [phase1, phase2], magn_colors=[c1,c2], phase_colors=[c1,c2], show_phase=False, names=["pocketvna", "nanovna"])
err_line1 = ax1.errorbar(freqs1, magn1_db, yerr=std_data1[3])
err_line2 = ax1.errorbar(freqs2, magn2_db, yerr=std_data2[3])
err_line1.set_label("pocketvna")
err_line2.set_label("nanovna")
ax1.grid(True)
ax2 = fig.add_subplot(2,1,2, projection="smith")
# Plotting smith charts
min_index1 = plot_res(ax=ax1, magn_db=magn1_db, freqs=freqs1, color="turquoise")
plot_smith(ax=ax2, reals=reals1, ims=ims1, c=c1, magn_db=magn1_db, c_spes="turquoise")
min_index2 = plot_res(ax=ax1, magn_db=magn2_db, freqs=freqs2, color="red")
...
print("STD of magnitude pocketvna = ", std_data1[3][min_index1-1:min_index1+2])
print("STD of magnitude nanovna = ", std_data2[3][min_index2-1:min_index2+2])
...
for i, values in enumerate([values_loaded_quad, values_unloaded_quad]):
    ...
    min_i_first = plot_res(ax, magn_db=values[1][0], freqs=freqs_q, color="red")
    min_i_switch = plot_res(ax, magn_db=values[1][3], freqs=freqs_q, color="darkblue")
